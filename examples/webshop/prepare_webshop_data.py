...
This script writes the fixed slime WebShop goal datasets used by the examples in
this directory:
...
def _build_slime_webshop_rows(
    ...
    goal_seed = int(args.env_seed)
    train_rows = [_record(goal_idx, "train", goal_seed=goal_seed) for goal_idx in range(train_start, goal_count)]
    valid_rows = [_record(goal_idx, "valid", goal_seed=goal_seed) for goal_idx in range(args.valid_size)]
