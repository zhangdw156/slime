...
def _dispose_rollout_manager(rollout_manager):
    if rollout_manager is None:
        return
    try:
        ray.get(rollout_manager.dispose.remote(), timeout=180)
    except Exception as e:
        logger.warning(f"Failed to dispose rollout manager cleanly: {e}")
    finally:
        try:
            ray.kill(rollout_manager, no_restart=True)
        except Exception as e:
            logger.warning(f"Failed to kill rollout manager actor: {e}")


def _dispose_train_group(group):
    if group is None:
        return
    dispose = getattr(group, "dispose", None)
    if dispose is None:
        return
    try:
        dispose()
    except Exception as e:
        logger.warning(f"Failed to dispose train group: {e}")


def _remove_placement_groups(pgs):
    if not pgs:
        return
    seen = set()
...
def train(args):
    ...
    try:
        ...
        _dispose_train_group(actor_model)
        _dispose_train_group(critic_model)
        _remove_placement_groups(pgs)
